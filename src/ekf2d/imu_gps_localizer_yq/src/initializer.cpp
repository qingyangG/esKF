...
bool Initializer::AddGpsPositionData(const GpsPositionDataPtr gps_data_ptr, State* state) {
    if (imu_buffer_.size() < kImuDataBufferLength) {
        LOG(WARNING) << "[AddGpsPositionData]: No enough imu data!";
        return false;
    }
    ...
    // Set covariance.
    state->cov.setZero();
    // state->cov.block<2, 2>(0, 0) = 100. * Eigen::Matrix3d::Identity(); // position std: 10 m
    // state->cov(2, 2) = 100. * kDegreeToRadian * 100. * kDegreeToRadian; // yaw std: 100 degree.
...
bool Initializer::ComputeG_R_IFromImuData(Eigen::Matrix3d* G_R_I) {
    ...
    // 如下的操作有一个假设， imu的z轴和重力平行.https://github.com/rpng/open_vins/blob/58afb2daf47498911100edc7452ccd7e42a2db50/ov_init/src/utils/helper.h#L138
