...
LocalizationWrapper::LocalizationWrapper(ros::NodeHandle& nh) {
    ...
    imu_gps_localizer_ptr_ = 
        std::make_unique<ImuGpsLocalization::ImuGpsLocalizer>(acc_noise, gyro_noise,
                                                              ...
                                                              I_p_Gps);
    /*
        1.接收数据, 在第一次收到gps数据时进行初始化:
            a.创建enu
            b.根据累计的imu数据, 给定imu关于enu的初始state. 
            c.初始化偏置为0, state的协方差矩阵.
        2.每次有imu数据, 就进行预测(回调函数ImuCallback), 位姿保存到nav_msgs::Path ros_path_,
    state_pub_将其发布出去. 
        3.每次有gps数据, 就进行矫正(GpsPositionCallback), 修正系统的当前状态量, 有利于后续预测.
    注意不止位置, 速度/rotation/还有偏置也更新了.
    */
    // Subscribe topics.
